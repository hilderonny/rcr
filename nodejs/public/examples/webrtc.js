...
function WebRTC(localMediaProperties, autoAcceptIncomingCall) {
    ...
    self.socket.on('socketConnected', function(socketIds) {
        self.sendEvent('socketConnected', socketIds);
    });

    self.socket.on('socketDisconnected', function(socketId) {
        self.sendEvent('socketDisconnected', socketId);
    });

    // Init local video capturing and send event
